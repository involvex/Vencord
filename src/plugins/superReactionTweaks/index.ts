...
export default definePlugin({
    ...
    patches: [
        {
            ...
            replacement: [
                // FIXME(Bundler minifier change related): Remove the non used compability once enough time has passed
                {
                    ...
                    replace: (_, rest, playingCount) =>
                        `${rest}!$self.shouldPlayBurstReaction(${playingCount})`,
                    noWarn: true,
                },
                {
                    /*
                    ...
                    replace: (_, rest, playingCount) =>
                        `${rest}if(!$self.shouldPlayBurstReaction(${playingCount}))return;`,
                },
            ],
        },
        {
    ...
    shouldPlayBurstReaction(playingCount: number) {
        ...
        if (settings.store.superReactionPlayingLimit > playingCount)
            return true;
        return false;
    },
