...
function filterTreeWithTargetNode(
    children: any,
...
const IS_BETTER_FOLDERS_VAR =
    "typeof isBetterFolders!=='undefined'?isBetterFolders:arguments[0]?.isBetterFolders";
const BETTER_FOLDERS_EXPANDED_IDS_VAR =
...
export default definePlugin({
    ...
    patches: [
        {
            ...
            replacement: [
                ...
                // Make the callback for returning the guild node component depend on isBetterFolders and betterFoldersExpandedIds
                {
                    ...
                    replace:
                        "$&,arguments[0]?.isBetterFolders,arguments[0]?.betterFoldersExpandedIds",
                },
                ...
                // If we are rendering the Better Folders sidebar, we filter out everything but the Guild List from the Sidebar children
                {
                    ...
                    replace:
                        "$&.filter($self.makeGuildsBarSidebarFilter(!!arguments[0]?.isBetterFolders))",
                },
            ],
        },
        {
    ...
    wrapGuildNodeComponent(
        ...
        return <div style={{ display: "none" }}>{originalComponent()}</div>;
    },
    ...
    makeGuildsBarGuildListFilter(isBetterFolders: boolean) {
        return (child: any) => {
            ...
            try {
                return (
                    child?.props?.["aria-label"] === getIntlMessage("SERVERS")
                );
            } catch (e) {
                console.error(e);
                return true;
    ...
    makeGuildsBarSidebarFilter(isBetterFolders: boolean) {
        return (child: any) => {
            ...
            try {
                return filterTreeWithTargetNode(
                    child,
                    child => child?.props?.renderTreeNode != null,
                );
            } catch (e) {
                console.error(e);
                return true;
