...
export type MemberListDecoratorFactory = (
    props: DecoratorProps,
) => JSX.Element | null;
type OnlyIn = "guilds" | "dms";

export const decoratorsFactories = new Map<
    string,
    { render: MemberListDecoratorFactory; onlyIn?: OnlyIn }
>();

export function addMemberListDecorator(
    identifier: string,
...
export function __getDecorators(
    props: DecoratorProps,
    type: "guild" | "dm",
): JSX.Element {
        ...
        ([key, { render: Decorator, onlyIn }]) => {
            ...
            return (
                <ErrorBoundary
                    noop
                    key={key}
                    message={`Failed to render ${key} Member List Decorator`}
                >
    ...
    return (
        <div className="vc-member-list-decorators-wrapper">{decorators}</div>
    );
}
