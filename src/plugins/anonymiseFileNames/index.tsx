...
export default definePlugin({
    ...
    patches: [
        ...
        },
        {
            ...
            replacement: {
                ...
                replace:
                    "arguments[0].canEdit!==false?$self.AnonymiseUploadButton(arguments[0]):null,",
            },
        },
    ],

    AnonymiseUploadButton: ErrorBoundary.wrap(
        ({ upload }: { upload: CloudUpload }) => {
            const [anonymise, setAnonymise] = useState(
                upload[ANONYMISE_UPLOAD_SYMBOL] ??
                    settings.store.anonymiseByDefault,
            );
    ...
    anonymise(upload: CloudUpload) {
        if (
            (upload[ANONYMISE_UPLOAD_SYMBOL] ??
                settings.store.anonymiseByDefault) === false
        ) {
            return;
        }
