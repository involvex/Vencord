...
export function ThemeSettingsComponent() {
    const currentTheme = useStateFromStores(
        [ThemeStore],
        () => ThemeStore.theme,
    );
    ...
    const nitroThemeEnabled = useStateFromStores(
        [NitroThemeStore],
        () => NitroThemeStore.gradientPreset != null,
    );
            ...
            {(contrastWarning || nitroThemeEnabled) && (
                <>
                    <ErrorCard className={Margins.top8}>
                        ...
                        {contrastWarning && (
                            <Forms.FormText>
                                {">"} Selected color won't contrast well with
                                text
                            </Forms.FormText>
                        )}
                        {nitroThemeEnabled && (
...
export function ResetThemeColorComponent() {
    return (
        <Button onClick={() => onPickColor(0x313338)}>Reset Theme Color</Button>
    );
}
