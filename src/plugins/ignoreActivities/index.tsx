...
function ToggleActivityComponent(activity: IgnoredActivity, isPlaying = false) {
    ...
    return ToggleIconOn(
        activity,
        isPlaying ? "var(--green-300)" : "var(--interactive-normal)",
    );
}
...
export default definePlugin({
    ...
    async start() {
        if (settings.store.ignoredActivities.length !== 0) {
            ...
            for (const [
                index,
                ignoredActivity,
            ] of settings.store.ignoredActivities.entries()) {
    ...
    isActivityNotIgnored(props: {
        ...
        if (props.application_id != null) {
            return (
                !settings.store.ignoredActivities.some(
            ...
            if (exePath) {
                return !settings.store.ignoredActivities.some(
