...
export type Button = ComponentType<ButtonProps> & {
    ...
    Sizes: Record<
        "NONE" | "SMALL" | "MEDIUM" | "LARGE" | "XLARGE" | "MIN",
        string
    >;
};
...
export type Slider = ComponentClass<
    PropsWithChildren<{
        initialValue: number;
        defaultValue?: number;
        keyboardStep?: number;
...
export type ListScrollerThin = ComponentType<
    ScrollerBaseProps & {
        sections: number[];
...
export type Clickable = <T extends "a" | "div" | "span" | "li" = "div">(
    props: PropsWithChildren<ComponentPropsWithRef<T>> & {
        tag?: T;
    },
) => ReactNode;
