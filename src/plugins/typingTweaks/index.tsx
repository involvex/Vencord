...
export default definePlugin({
    ...
    patches: [
        {
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace:
                        "let typingUserObjects = $self.useTypingUsers(arguments[0]?.channel);",
                },
                {
    ...
    useTypingUsers(channel: Channel | undefined): User[] {
        try {
            ...
            const myId = useStateFromStores([AuthenticationStore], () =>
            ...
            return Object.keys(typingUsers)
                .filter(
                    id =>
                        id &&
                        id !== myId &&
                        !RelationshipStore.isBlockedOrIgnored(id),
                )
    ...
    renderTypingUsers: ErrorBoundary.wrap(
        ({
            ...
            users,
            children,
        }: PropsWithChildren<{ guildId: string; users: User[] }>) => {
            try {
                ...
                return children.map(c => {
                    if (
                        c.type !== "strong" &&
                        !(typeof c !== "string" && !React.isValidElement(c))
                    )
                        return c;
                ...
                new Logger("TypingTweaks").error(
                    "Failed to render typing users:",
                    e,
                );
            }
