...
function getCompatReplacement(key: string) {
    ...
    const legacyHashed = getReplacement(
        false,
        runtimeHashMessageKeyLegacy(key),
    );
...
export function canonicalizeMatch<T extends RegExp | string>(
    match: T,
): T extends RegExp ? RegExp : string | RegExp {
