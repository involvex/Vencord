...
export default definePlugin({
    ...
    shouldHide(props: MessageDeleteProps): boolean {
        try {
            ...
            const is = (key: string) =>
                collapsedReason === i18n.t[runtimeHashMessageKey(key)]() ||
                collapsedReason === i18n.t[runtimeHashMessageKeyLegacy(key)]();

            return (
                is("BLOCKED_MESSAGE_COUNT") ||
                (settings.store.applyToIgnoredUsers &&
                    is("IGNORED_MESSAGE_COUNT"))
            );
        } catch (e) {
            new Logger("NoBlockedMessages").error(
